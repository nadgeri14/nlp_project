...
class HgcnInput:
    def __init__(self, manifold, c, device, cuda, feat_dim, model, num_layers, dropout, bias, use_att, local_agg, dim, n_nodes, act, task):
...
class ASTGPOLS(nn.Module):
    ...
    def __init__(self, nfeat, nhid_graph, nhid_gru, nclass, dropout, nheads, graph_layer=True, rnn_layer='gru', att_layer=True, gnn_name='gat'):
        ...
        if graph_layer:
            ...
            if gnn_name == 'gat':
                ...
                hgcn_input = HgcnInput(manifold='PoincareBall', c=1.0, device=DEVICE, cuda=1, feat_dim=nfeat, model='HGCN', num_layers=2,
                      dropout=dropout, bias=1, use_att=0, local_agg=0, dim=nhid_graph, n_nodes=200, act='relu', task='nc')
                self.gnn = hgcn.BaseModel(hgcn_input).to(DEVICE).double()
            else:
    ...
    def forward(self, graph, fts, time_steps, adj=None):
        ...
        if self.gnn_name == 'hgcn' and adj is None:
            raise RuntimeError("HGCN graph needs the adjacency matrix as input")
        ...
        if self.graph_layer:
            for i in range(time_steps):
                x = fts[i]
                if self.gnn_name == 'hgcn':
                    G = torch.tensor(adj[i]).to(DEVICE)
                    y = F.leaky_relu(self.gnn.encode(x.double(), G).float(), 0.2)
                else:
                    G = graph[i]

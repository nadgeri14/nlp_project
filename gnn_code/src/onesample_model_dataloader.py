...
parser.add_argument("--n_users", dest="n_users", default=-1, type=int)
parser.add_argument("--n_train_samples", dest="n_train_samples", default=1, type=int)
parser.add_argument("--n_val_samples", dest="n_val_samples", default=1, type=int)
...
parser.add_argument("--user_ids", dest="user_ids", required=True, type=str)
...
for graph in path_sort(
    ...
    doc_sum = 0
    users = 0
    for _, row in RedditUserDataset.load_from_instance_file(graph).data_frame.iterrows():
        users += 1
...
if not args.random_features:
    ...
    for frame in timeframed_dataset:
        ...
        feature_map = frame.compute_features(embedder, embed_mode=embed_mode)
    ...
    for frame in timeframed_dataset:
        ...
        for _, row in frame.data_frame.iterrows():
            precomputed_features[row['user_id']].append(torch.tensor(np.random.uniform(low=-1.5, high=1.5, size=768)))
...
for n in range(n_train_samples):
    seed_counter += 1
    sample_ids = train_sample_frame['user_id']
...
for n in range(n_val_samples):
    seed_counter += 1
    sample_ids = val_sample_frame['user_id']
    sample_frames = [frame.filter_user_ids(sample_ids, inplace=False) for frame in timeframed_dataset]

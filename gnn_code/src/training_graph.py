...
parser.add_argument("--nhid_gru", dest="nhid_gru", default=64, type=int)
...
parser.add_argument("--graph_layer", dest="graph_layer", default=True, type=str2bool)
parser.add_argument("--att_layer", dest="att_layer", default=True, type=str2bool)
parser.add_argument("--rnn_layer", dest="rnn_layer", default='gru', type=str)
parser.add_argument("--merge_samples", dest="merge_samples", default=False, type=str2bool)
...
model = ASTGPOLS(nfeat=users_dim, nhid_graph=args.nhid_graph, nhid_gru=args.nhid_gru, nclass=2, dropout=dropout,
                nheads=nheads, graph_layer=args.graph_layer, rnn_layer=args.rnn_layer, att_layer=args.att_layer, gnn_name=args.gnn).to(DEVICE)
...
for sample in train_samples:
    sample.build_adj_matrix()
for sample in val_samples:
    sample.build_adj_matrix()
for sample in test_samples:
    sample.build_adj_matrix()

if args.merge_samples and not args.lazy_loading:
    print("Running experiment with merged samples")
    merge_samples(train_samples)
    merge_samples(val_samples)
    merge_samples(test_samples)
elif args.merge_samples and args.lazy_loading:
    print("Cannot merge samples with lazy loading on.")
    sys.exit()
...
for i in range(max_epochs):
    ...
    if metrics['val_acc'] >= top_val_acc:
        ...
        no_improvement_epochs = 0
        best_res = metrics
        #conf = calc_confuscalc_confusion_matricesion_matrices(model, n_train=n_train, train_path=train_path, n_test=n_test, test_path=test_path, n_val=n_val, val_path=val_path)
        #train_preds, val_preds, test_preds = calc_prediction_map(model, n_train=n_train, train_path=train_path, n_test=n_test, test_path=test_path, n_val=n_val, val_path=val_path)
        save_checkpoint({
            'epoch': i + 1,
...
result_obj['gru_nhid'] = args.nhid_gru
...
result_obj['graph_layer'] = args.graph_layer
result_obj['gnn_type'] = args.gnn
result_obj['rnn_layer'] = args.rnn_layer
result_obj['att_layer'] = args.att_layer
result_obj['gnn'] = args.gnn
result_obj['merge_samples'] = args.merge_samples
